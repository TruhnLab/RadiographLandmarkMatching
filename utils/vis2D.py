...
def save_plot(path, close_fig=True, **kw):
...
def save_assignments(assignments, matches0, matches1, save_path='', identifier='test'):
        m,n = assignments.shape
        output = np.zeros((m+1,n+1))
        output[:m,:n] = assignments
        output[:m,-1] = matches0
        output[-1,:n] = matches1
        io.imsave(os.path.join(save_path, f'{identifier}.tif'), output, check_contrast=False)
...
def plot_image(image, trgt_landmarks, prd_landmarks, path=None, labels=None, sizes=None, alpha=0.5):

    fig, ax = plt.subplots(1, 1, figsize=[10,10])
    plt.imshow(image, cmap='gray')
        
    # Draw landmarks
    ax.scatter(trgt_landmarks[:, 0], trgt_landmarks[:, 1], c='green', s=sizes, alpha=alpha)
    ...
    for trgt_landmark, prd_landmark in zip(trgt_landmarks, prd_landmarks):
        ax.plot([trgt_landmark[0], prd_landmark[0]], [trgt_landmark[1], prd_landmark[1]], 'k-', alpha=0.5)
